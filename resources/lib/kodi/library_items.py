...
def _get_item(mediatype, filename):
    ...
    fname = common.make_legal_filename(filename)
...
def get_previously_exported_items():
    ...
    for folder in _lib_folders(FOLDER_MOVIES) + _lib_folders(FOLDER_TV):
        for filename in xbmcvfs.listdir(folder)[1]:
            filepath = g.py2_decode(common.make_legal_filename('/'.join([folder, filename])))
...
def _lib_folders(section):
    section_dir = g.py2_decode(xbmc.translatePath(common.make_legal_filename('/'.join([library_path(), section]))))
    return [g.py2_decode(common.make_legal_filename('/'.join([section_dir, folder.decode('utf-8')])))
            for folder
...
def export_item(item_task, library_home):
    ...
    destination_folder = g.py2_decode(common.make_legal_filename('/'.join(
        [library_home, item_task['section'], item_task['destination']])))
    _create_destination_folder(destination_folder)
    if item_task['is_strm']:
        export_filename = g.py2_decode(common.make_legal_filename('/'.join(
    ...
    if item_task['nfo_data'] is not None:
        nfo_filename = g.py2_decode(common.make_legal_filename('/'.join(
...
def remove_item(item_task, library_home=None):
    ...
    try:
        ...
        tvshow_nfo_file = g.py2_decode(common.make_legal_filename('/'.join([parent_folder, 'tvshow.nfo'])))
